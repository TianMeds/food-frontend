...
const Navbar = () => {
    ...
    return (
        ...
        <div>
            <img src='https://raw.githubusercontent.com/TianMeds/image--stocks-for-coding/274a634a12930720116a85d65cf5930559da76bf/G%26R%20Eatery%20Logo.svg' alt='Your Logo' className='h-28 w-28' />
            </div>
        ...
        <div className='focus:outline-none mx-4'>
          ...
          {navigation.filter(item => item.title !== 'Order Now' && item.title !== 'Login').map((item) => (
            <Link key={item.title} href={item.link} className={`relative group overflow-hidden ${pathName === item.link && 'text-blue-700 underline'}`}>
              {item.title}
              <span
                className={`w-full h-[1px] inline-flex absolute bottom-0 left-0 bg-black dark:bg-white -translate-x-[105%] duration-300 ${
                  pathName === item.link ? 'bg-black dark:bg-black' : 'bg-black dark:bg-white'
                }`}
              />
            </Link>
          ))}
            ...
            {navigation.slice(-2).map((item) => (
              ...
              >
                ...
                {pathName === item.link && (
                  <span
                    className={`w-full h-[1px] inline-flex absolute bottom-0 left-0 bg-black dark:bg-white -translate-x-[105%] ${
                      pathName === item.link ? 'bg-blue-600 dark:bg-blue-600 translate-x-0' : 'bg-black dark:bg-white'
                    }`}
                  />
                )}
              </Link>
            ...
            <div className='flex flex-col justify-center items-center h-full w-full'>
              ...
              {navigation.map((item) => (
                <Link key={item.title} href={item.link} className={`relative group overflow-hidden ${pathName === item.link && 'text-blue-700'} text-4xl mb-2 text-white`}>
                  {item.title}
                  <span
                    className={`w-full h-[1px] inline-flex absolute bottom-0 left-0 bg-black dark:bg-white -translate-x-[105%] group-hover:translate-x-0 duration-300 ${
                      pathName === item.link ? 'bg-blue-600 dark:bg-blue-600 translate-x-0' : 'bg-black dark:bg-white'
                    }`}
                  />
                </Link>
